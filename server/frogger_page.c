...
#define FROGGER_CONTENT "<html><body>Frogger! <button id=\"demo\">Button</button></body><script>document.getElementById(\"demo\").onclick = function() {myFunction()};function myFunction() { const request = new Request(\"/frogger.html/\", { method: \"POST\", body: 'd', }); fetch(request).then((response) => { if (response.status !== 200) { throw new Error(\"Something went wrong on API server!\"); } return response; }).catch((error) => { console.error(error); });};</script></html>"
